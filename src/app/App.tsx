...
export default function App() {
  ...
  const onPaletteImageUpload = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setStatus(`Extracted ${palette.length} colors from ${file.name}.`);
      e.target.value = '';
    } catch (error) {
    ...
    <div className={panelClass}>
      ...
      <main className="body">
        <aside className="sidebar">
          ...
          <label className="upload-label">Upload image for palette<input type="file" accept="image/*" onChange={onPaletteImageUpload} /></label>
          ...
          {palettePreview.length > 0 && (
            <>
              <p className="mini-label">Extracted palette{paletteSourceName ? ` from ${paletteSourceName}` : ''}</p>
              <button onClick={() => navigator.clipboard.writeText(palettePreview.join(', '))}>Copy Palette</button>
